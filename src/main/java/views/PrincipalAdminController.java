...
//Botón para ver los eventos
   ...
   * @throws IOException Si ocurre un error de E/S al cargar la vista.
   */
  void initVerCuenta(Stage stage, VerCuentaController verCuentaController, String nombre, BorderPane root, String tipoCuenta) throws IOException {
    this.stage = stage;
  ...
  private void escenaAddEvento() throws IOException {
    ...
    BorderPane addEvento = loader.load();
    // Se obtiene la escena actual del stage y se establece el nuevo contenido
    Scene escena = stage.getScene();
    BorderPane root = (BorderPane) escena.getRoot();
    root.getChildren().clear();
    root.getChildren().add(addEvento);
  }
